...
- [x] Travel page rental car coordination: guests enter arrival/departure dates and find others to split a rental car with (double opt-in email, guests choose scope). Built; needs `scripts/travel-plans.sql` run in Supabase to go live. See [pages/travel-coordination.md](pages/travel-coordination.md)
- [ ] **Admin view of rental-car coordination**: an admin dashboard page (or section) listing guests who've added travel plans (`travel_plans`) so Julia & Jon can see who's coordinating, arrival/departure windows, and intent at a glance. Read-only; reuses the admin auth + layout. See [pages/travel-coordination.md](pages/travel-coordination.md)
